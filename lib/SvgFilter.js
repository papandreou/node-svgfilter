...
SvgFilter.prototype.end = function (chunk) {
    ...
    const svgAssetUrl = this.options.url || assetGraph.root + 'fakeFileName.svg';
    ...
    const externalScriptFileNamesToRun = arrayify(this.options.runScript).filter(function (externalScriptFileNameOrScriptId) {
    ...
    const assetConfigsToLoad = [
        ...
        ...externalScriptFileNamesToRun.map(
            scriptFileName => ({
                scriptFileName,
    ...
    (async () => {
        try {
            ...
            function runJavaScriptAsset(javaScript, context) {
            ...
            if (this.options.runScript) {
                ...
                for (const svgScript of assetGraph.findRelations({from: svgAsset, type: 'SvgScript'})) {
                    if (this.options.runScript === true || runScriptIds.indexOf(svgScript.node.getAttribute('id')) !== -1) {
                        runJavaScriptAsset(svgScript.to, context);
                    }
                }

                for (const scriptFileName of externalScriptFileNamesToRun) {
                    const javaScripts = assetGraph.findAssets({scriptFileName: scriptFileName, isLoaded: true});
                    if (javaScripts.length === 1) {
                        const javaScript = javaScripts[0];
                        this.emit('etagFragment', javaScript.md5Hex);
                        runJavaScriptAsset(javaScript, context);
                    } else {
