...
SvgFilter.prototype.end = function(chunk) {
  ...
  const externalScriptFileNamesToRun = arrayify(this.options.runScript).filter(
    externalScriptFileNameOrScriptId => /\.js$/.test(externalScriptFileNameOrScriptId)
  );
      ...
      if (this.options.runScript) {
        ...
        for (const scriptFileName of externalScriptFileNamesToRun) {
          ...
          if (javaScripts.length === 1) {
            ...
            reportError(
              new Error(
                'Unexpected number of JavaScript assets found for ' +
                  scriptFileName +
                  ': ' +
                  javaScripts.length
              )
